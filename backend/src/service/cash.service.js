...
export const getCashMovements = async (filters) => {
    const where = {};
    let rangeFilter;
    ...
    const paymentWhere = rangeFilter
        ? {
            paymentDate: rangeFilter,
        }
        : {};
    const loanWhere = rangeFilter
        ? {
    ...
    const paymentIdByKey = new Map(legacyPayments.map((payment) => [
        buildMovementKey(payment.loanId, payment.amount, payment.paymentDate),
        payment.id,
    ]));
    const existingIncomeKeys = new Set(cashMovements
        .filter((movement) => movement.type === "INCOME" && movement.loanId)
        .map((movement) => buildMovementKey(movement.loanId, movement.amount, movement.createdAt)));
    const existingExpenseKeys = new Set(cashMovements
    ...
    const normalizedCashMovements = cashMovements.map((movement) => ({
        ...movement,
        paymentId: movement.type === "INCOME" && movement.loanId
    ...
    return [...normalizedCashMovements, ...synthesizedPaymentMovements, ...synthesizedLoanMovements]
        .sort((firstMovement, secondMovement) => new Date(secondMovement.createdAt).getTime() -
        new Date(firstMovement.createdAt).getTime());
};
function buildMovementKey(loanId, amount, date) {
    return `${loanId}:${roundMoney(amount)}:${date.toISOString().slice(0, 10)}`;
}
function roundMoney(value) {
    return Number(value.toFixed(2));
}
function buildRangeFilter(filters) {
    if (filters.day) {
