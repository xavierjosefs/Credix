...
import { addDays, applyPaymentToInstallments, applyPrincipalPaymentToSegments, calculateInstallmentSchedule, calculateLoanAccruedInterest, calculateNextDueDate, calculateRemainingInstallmentInterest, diffInDays, enrichLoan, getNextPendingInstallmentDueDate, getLoanWithRelationsById, getPeriodDays, loanRelationsInclude, resolveInstallmentLoanStatus, resolveLoanStatus, roundMoney, } from "./loan.helpers.js";
export const createLoan = async (data, adminId) => {
    const { clientId, principalAmount, interestRate, frequency, startDate, method, existingLoanId, installmentCount, type = LoanType.FLEXIBLE, } = data;
...
export const registerLoanPayment = async (data, adminId) => {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (loan.type === LoanType.INSTALLMENT) {
            ...
            const { installments, appliedAmount, interestPaid, principalPaid } = applyPaymentToInstallments(loan, amount);
            if (appliedAmount <= 0) {
        ...
        const accruedInterest = calculateLoanAccruedInterest(loan, effectivePaymentDate, interestDays);
        if (amount < accruedInterest) {
            throw new Error(`Payment must cover at least accrued interest (${accruedInterest})`);
        }
        const interestPaid = roundMoney(accruedInterest);
        const principalPaid = roundMoney(Math.min(Math.max(amount - interestPaid, 0), loan.remainingBalance));
        const newRemainingBalance = roundMoney(Math.max(loan.remainingBalance - principalPaid, 0));
        ...
        if (interestCoverageDate >= loan.nextDueDate) {
            nextDueDate = calculateNextDueDate(loan.nextDueDate, interestCoverageDate, loan.frequency);
        }
