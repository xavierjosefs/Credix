...
type CashMovementRecord = Awaited<ReturnType<typeof getCashMovements>>[number];

let browserPromise: Promise<Browser> | null = null;

export async function generateClientCollectionReportPdf(
  filters: ClientReportFiltersDto
): Promise<PdfDocument> {
        ...
        <div class="totals-panel">
          ...
          <div class="grand-total">
            ...
            <strong>${formatCurrency(
              report.summary.totalCapitalPending + report.summary.totalInterestPending
            )}</strong>
          </div>
        </div>
      </section>
...
export async function generateCashReportPdf(
  ...
  const summary = filteredMovements.reduce(
  ...
  const html = buildHtmlDocument({
    ...
    body: `
      ${renderReportHeader({
        title: "REPORTE DE CAJA",
        subtitle: `${escapeHtml(formatCashRange(filters))} · Administrador: ${escapeHtml(
          formatCashAdminLabel(filters.admin, adminScopedMovements)
        )}${filters.q?.trim() ? ` · Busqueda: ${escapeHtml(filters.q.trim())}` : ""}`,
      })}
      ${renderSummaryGrid([
...
export async function generatePaymentReceiptPdf(input: {
  ...
  const html = buildHtmlDocument({
    ...
    body: renderReceiptLayout({
      ...
      footerText:
        "Este documento es un comprobante oficial emitido por Credix. Conservar este recibo para cualquier consulta posterior sobre el pago realizado.",
    }),
  });
...
export async function generateDisbursementReceiptPdf(
  loanId: string,
  ...
  const laterTopUpAmount = expenseMovements
    .filter(
      (cashMovement: {
        ...
        cashMovement.createdAt > issuedAt &&
        cashMovement.description === "Monto agregado a prestamo existente"
    )
    .reduce(
      (sum: number, cashMovement: { amount: number }) => sum + cashMovement.amount,
      0
    );
  ...
  const balanceAfterOperation = roundMoney(
    loan.remainingBalance + principalRecoveredAfter - laterTopUpAmount
  );
  const previousBalance =
    mode === "TOPUP" ? roundMoney(Math.max(balanceAfterOperation - amount, 0)) : 0;
  ...
  const html = buildHtmlDocument({
    ...
    body: renderReceiptLayout({
      ...
      totalValue: amount,
      footerText:
        "Este documento respalda la salida de fondos registrada para este prestamo. Conservar este comprobante para cualquier consulta posterior.",
    }),
  });
...
function renderReportHeader({ title, subtitle }: { title: string; subtitle: string }) {
...
function renderSummaryGrid(cards: Array<{ label: string; value: string; dark?: boolean }>) {
  return `<section class="summary-grid">${cards
    .map(
      (card) => `<article class="summary-card ${card.dark ? "dark" : ""}"><label>${escapeHtml(card.label)}</label><strong>${escapeHtml(card.value)}</strong></article>`
    )
    .join("")}</section>`;
}

function renderResumeRow(label: string, value: string) {
...
function filterCashMovementsByAdmin(movements: CashMovementRecord[], adminFilter?: string) {
...
function filterCashMovementsByQuery(movements: CashMovementRecord[], searchQuery?: string) {
...
function resolveDisbursementMovement(
  ...
  if (parsedIssuedAt) {
    ...
    const closestByDate = expenseMovements
      .filter((movement) =>
        parsedAmount === null ? true : roundMoney(movement.amount) === roundMoney(parsedAmount)
      )
      .sort(
        (firstMovement, secondMovement) =>
          Math.abs(firstMovement.createdAt.getTime() - targetTimestamp) -
          Math.abs(secondMovement.createdAt.getTime() - targetTimestamp)
      )[0];
...
function formatCashRange(filters: CashReportFilters) {
  if (filters.day) {
...
function formatCashAdminLabel(adminFilter: string | undefined, movements: CashMovementRecord[]) {
  if (!adminFilter || adminFilter === "ALL") {
    return "Todos";
  }
...
function formatCashRefId(id: string) {
  return `TRX-${id.replace(/^(legacy-payment-|legacy-loan-)/, "").slice(0, 4).toUpperCase()}`;
}

function formatCashMovementType(movement: CashMovementRecord) {
  if (movement.type === "INCOME") {
    return "Pago";
  }
...
function formatPaymentMethod(method: PaymentMethod | "UNKNOWN" | null | undefined) {
...
function formatLoanLabel(type: LoanType, frequency: string) {
...
function formatCollectionMethod(value: string | null) {
  switch (value) {
    case "CAJERO":
      return "Cajero";
    case "DEPOSITO":
...
function formatInstitution(value: string | null) {
  switch (value) {
    case "POLICIA":
      return "Policia";
    case "PENSIONADO":
...
function formatInputDay(value: string) {
...
function buildSafeDateStamp(date: Date) {
