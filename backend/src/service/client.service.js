import { LoanStatus, LoanType, PaymentFrequency, Prisma, } from '@prisma/client';
import prisma from '../prisma/prisma.js';
import { encrypt } from '../utils/encryption.js';
import { isValidPhone } from '../utils/validators/phone.js';
import { buildClientSearchFilters, clientRelationsInclude, decryptClientCredentials, decryptManyClientCredentials, } from './client.helpers.js';
import { calculateLoanAccruedInterest, calculateRemainingInstallmentInterest, loanRelationsInclude, roundMoney, } from './loan.helpers.js';
...
const validCredentialBanks = [
    'BANRESERVAS',
    'POPULAR',
    'BHD',
    'CARIBE',
];
const validCollectionMethods = [
    'CAJERO',
    'DEPOSITO',
    'EFECTIVO',
    'TRANSFERENCIA',
];
const validPaymentFrequencies = ['MONTHLY', 'BIWEEKLY'];
export const createClient = async (data) => {
    const { name, cedula, address, birthDate, email, phone, phone2, phoneCompany, clientNumber, devicePhone, collectionMethod, profileImage, institution, credentials, bankAccounts, } = data;
...
export const getAllClients = async (data) => {
    const { cedula, name, email } = data ?? {};
    const orConditions = buildClientSearchFilters({ cedula, name, email });
    const resultClients = orConditions.length > 0
...
export const getClientCollectionReport = async (filters) => {
    ...
    const rows = Array.from(rowsByClient.values()).sort((first, second) => first.clientName.localeCompare(second.clientName, 'es'));
    return {
        filters: {
...
function getLoanPendingPrincipal(loan) {
    if (loan.type === LoanType.INSTALLMENT) {
        return roundMoney(loan.installments.reduce((sum, installment) => {
            ...
            const remainingPrincipal = roundMoney(Math.max(installment.principalPortion - principalCoveredBefore, 0));
            return sum + remainingPrincipal;
        }, 0));
    }
    return roundMoney(loan.remainingBalance);
}
function getLoanPendingInterest(loan) {
    if (loan.type === LoanType.INSTALLMENT) {
